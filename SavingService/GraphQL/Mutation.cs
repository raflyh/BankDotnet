...
namespace SavingService.GraphQL
{
    public class Mutation
    {
            ...
            ClaimsPrincipal claimsPrincipal)
        {
            ...
            if (balance != null)
            {
                if (savings.TotalSaving > input.Saving)
                {
                    TimeSpan result = DateTime.Now.Subtract(savings.Date);
                    savings.Date = DateTime.Now;
                    savings.TotalSaving = savings.TotalSaving + (result.TotalMinutes * 0.001 * savings.TotalSaving);
                    ...
                    try
                    {
                        if (savings.BalanceId != 0)
                        {
                            ...
                            transaction.Commit();
                            return await Task.FromResult(new OutputSaving("Succes Transfer Saving To Balance ", savings.TotalSaving, savings.Date));
                ...
                {
                    return await Task.FromResult(new OutputSaving("Transfer Saving To Balance Unsuccesfully, Saving Insufficient", input.Saving, DateTime.Now));
                }
            }

            return await Task.FromResult(new OutputSaving("Transfer Saving To Balance Unsuccesfully", input.Saving, DateTime.Now));
        }
